...
public class S3RetryingInputStreamTests extends OpenSearchTestCase {
    ...
    public void testInputStreamFullyConsumed() throws IOException {
        ...
        final S3RetryingInputStream stream = createInputStream(expectedBytes, 0L, Long.MAX_VALUE - 1);
    ...
    private S3RetryingInputStream createInputStream(final byte[] data, final Long position, final Long length)
        throws IOException {
//        final S3Object s3Object = new S3Object();
        Long end = Math.addExact(position, length - 1);
        Range range = new Range(position, end);
        final S3Client client = mock(S3Client.class);
        when(client.getObject(any(GetObjectRequest.class))).thenReturn(new ResponseInputStream<>(
            GetObjectResponse.builder()
                .contentLength(length)
                .contentRange(range.getHttpRangeHeader())
            .build(),
        ...
        return new S3RetryingInputStream(blobStore, "_blob", position, end);
    }
}
